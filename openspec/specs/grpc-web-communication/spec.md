...
系統 SHALL 提供一個獨立的 `BlazorN10WasmLab.Shared` 類別庫專案，存放 gRPC 服務介面與資料模型，同時被 Server 與 Client 專案參照。

#### Scenario: Shared project referenced by both server and client
- **WHEN** 開發者在 Shared 專案修改 `IWeatherService` 介面
